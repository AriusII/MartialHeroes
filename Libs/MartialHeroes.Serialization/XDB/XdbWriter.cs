...
/// <summary>
///     Writes a single XDB record into a destination byte span.
/// </summary>
...
public delegate void XdbRecordWriter<in T>(T record, Span<byte> destination);

/// <summary>
///     Writes fixed-size XDB binary records to a byte array.
///     XDB files contain no header — the output is a flat array of record bytes.
/// </summary>
public static class XdbWriter
{
	/// <summary>
	///     Serializes all records into a raw XDB byte array.
	/// </summary>
	/// <typeparam name="T">The record type to serialize.</typeparam>
	...
	public static byte[] WriteAll<T>(IReadOnlyList<T> records, int recordSize, XdbRecordWriter<T> writer)
	{
